...
def run_faster_whisper(files, output_dir: Path, model_name: str, device: str,
    ...
    # batch_size > 0 включает BatchedInferencePipeline — модель обрабатывает
    # сразу пачку кусков аудио за один проход по GPU вместо одного за раз.
    # Это и есть способ реально нагрузить простаивающую VRAM и ускориться
    # в несколько раз (в тестах SYSTRAN: large-v3, batch_size=8 — 63с вместо 17с
    # на том же аудио; batch_size=16 применим при большом запасе VRAM, как у вас).
    # batch_size=0 отключает батчинг — старое поведение "один кусок за раз".
    batched_model = None
    if batch_size and batch_size > 0:
        from faster_whisper import BatchedInferencePipeline
        batched_model = BatchedInferencePipeline(model=model)
        log(f"Батчинг включён: batch_size={batch_size} "
            f"(при ошибке 'out of memory' — запустите с меньшим --batch-size)")
        ...
        try:
            ...
            if batched_model is not None:
                segments, info = batched_model.transcribe(
                    str(mp3_path), batch_size=batch_size, **transcribe_kwargs
                )
            else:
...
def main():
    ...
    parser.add_argument("--batch-size", type=int, default=16,
                         help="Только faster-whisper: сколько кусков аудио обрабатывать за один проход "
                              "по GPU одновременно (batch). Больше = быстрее, но больше VRAM. "
                              "0 — отключить батчинг (старое поведение). По умолчанию 16 — "
                              "при 16 ГБ VRAM и текущих ~3.4 ГБ есть большой запас, можно пробовать "
                              "24-32. Если упадёт 'out of memory' — уменьшайте вдвое.")
    ...
    if args.engine == "faster-whisper":
        ...
        run_faster_whisper(files, output_dir, model_name, args.device,
                            args.compute_type, args.language, args.overwrite, args.batch_size)
    else:
